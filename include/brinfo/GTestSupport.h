...
#define BRINFO_CALL_F(expr, func_hash)                                         \
  ([&]() {                                                                     \
    ::BrInfo::Runtime::BeginInvocation(__FILE__, __LINE__, #expr,              \
                                       (func_hash));                           \
    ::BrInfo::Testing::InvocationScopeGuard _brinfo_scope_guard_{};            \
    return (expr);                                                             \
  }())
...
#define BRINFO_ASSERTION_END() ::BrInfo::Runtime::AssertionEnd()

// Example wrappers (non-intrusive): define new macros instead of overriding
// gtest's.
#ifdef BRINFO_HAS_GTEST
#define BRINFO_EXPECT_EQ(a, b)                                                 \
  do {                                                                         \
    ::BrInfo::Runtime::AssertionBegin("EXPECT_EQ", __FILE__, __LINE__,         \
                                      #a ", " #b);                             \
    EXPECT_EQ(a, b);                                                           \
...
//   BrInfo::Testing::GTestListener());
#ifdef BRINFO_HAS_GTEST
class GTestListener : public ::testing::EmptyTestEventListener {
public:
  void OnTestStart(const ::testing::TestInfo &info) override {
    ::BrInfo::Runtime::BeginTest(info.test_suite_name(), info.name(),
                                 info.file(), info.line());
  }
  ...
  void OnTestPartResult(const ::testing::TestPartResult &result) override {
    ...
    unsigned line = result.line_number() > 0
                        ? static_cast<unsigned>(result.line_number())
                        : 0u;
